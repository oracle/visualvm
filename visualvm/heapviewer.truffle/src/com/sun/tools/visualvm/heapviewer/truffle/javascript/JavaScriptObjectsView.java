 ...
 */
class JavaScriptObjectsView extends HeapViewerFeature {
    ...
    public JavaScriptObjectsView(HeapContext context, HeapViewerActions actions) {
        super("javascript_objects", "Objects", "Objects", JavaScriptSupport.createBadgedIcon(LanguageIcons.CLASS), 200);
        
        this.context = context;
        Heap heap = context.getFragment().getHeap();
        
        objectsView = new PluggableTreeTableView("javascript_objects", context, actions, TreeTableViewColumn.classes(heap, true)) {
            protected HeapViewerNode[] computeData(RootNode root, Heap heap, String viewID, HeapViewerNodeFilter viewFilter, List<DataType> dataTypes, List<SortOrder> sortOrders, Progress progress) {
                switch (getPreset()) {
                    case ALL_OBJECTS:
                        switch (getAggregation()) {
                            case TYPES:
                                return JavaScriptObjectsProvider.getAllObjects(root, JavaScriptObjectsView.this.context, viewID, viewFilter, dataTypes, sortOrders, progress, 1);
                            default:
                                return JavaScriptObjectsProvider.getAllObjects(root, JavaScriptObjectsView.this.context, viewID, viewFilter, dataTypes, sortOrders, progress, 0);
                        }
                    case DOMINATORS:
                        switch (getAggregation()) {
                            case TYPES:
                                return JavaScriptObjectsProvider.getDominators(root, JavaScriptObjectsView.this.context, viewID, viewFilter, dataTypes, sortOrders, progress, 1);
                            default:
                                return JavaScriptObjectsProvider.getDominators(root, JavaScriptObjectsView.this.context, viewID, viewFilter, dataTypes, sortOrders, progress, 0);
                        }
                    case GC_ROOTS:
                        switch (getAggregation()) {
                            case TYPES:
                                return JavaScriptObjectsProvider.getGCRoots(root, JavaScriptObjectsView.this.context, viewID, viewFilter, dataTypes, sortOrders, progress, 1);
                            default:
                                return JavaScriptObjectsProvider.getGCRoots(root, JavaScriptObjectsView.this.context, viewID, viewFilter, dataTypes, sortOrders, progress, 0);
                        }
                    default:
