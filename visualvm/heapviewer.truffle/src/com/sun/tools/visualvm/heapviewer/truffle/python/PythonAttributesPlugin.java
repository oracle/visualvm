 ...
 * @author Tomas Hurka
 */
class PythonAttributesPlugin extends HeapViewPlugin {
    ...
    PythonAttributesPlugin(HeapContext context, HeapViewerActions actions) {
        super("Attributes", "Attributes", Icons.getIcon(ProfilerIcons.NODE_FORWARD));

        heap = context.getFragment().getHeap();

        objectsView = new TreeTableView("python_objects_attributes", context, actions, TreeTableViewColumn.instancesMinimal(heap, false)) {
            @Override
            protected HeapViewerNode[] computeData(RootNode root, Heap heap, String viewID, HeapViewerNodeFilter viewFilter, List<DataType> dataTypes, List<SortOrder> sortOrders, Progress progress) {
                if (selected != null) {
                    List<FieldValue> fields = selected.getAttributes();

                    HeapViewerNode[] nodes = getNodes(fields, root, heap, viewID, viewFilter, dataTypes, sortOrders, progress);
                    return nodes == null || nodes.length == 0 ? new HeapViewerNode[] { new TextNode("<no attributes>") } : nodes;
                }
    ...
    protected HeapViewerNode[] getNodes(List<FieldValue> fields, HeapViewerNode parent, Heap heap, String viewID, HeapViewerNodeFilter viewFilter, List<DataType> dataTypes, List<SortOrder> sortOrders, Progress progress) {
        return PythonAttributesProvider.getNodes(fields, parent, heap, viewID, viewFilter, dataTypes, sortOrders, progress);
    }
    ...
    public static class Provider extends HeapViewPlugin.Provider {
        ...
        public HeapViewPlugin createPlugin(HeapContext context, HeapViewerActions actions, String viewID) {
            if (PythonHeapFragment.isPythonHeap(context)) {
                return new PythonAttributesPlugin(context, actions);
            }
            return null;
        }
    }
