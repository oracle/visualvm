...
public abstract class TruffleFieldsPlugin extends HeapViewPlugin {
    ...
    public TruffleFieldsPlugin(String name, String description, String viewID, HeapContext context, HeapViewerActions actions) {
        ...
        objectsView = new TreeTableView(viewID, context, actions, TreeTableViewColumn.instancesMinimal(heap, false)) {
            protected HeapViewerNode[] computeData(RootNode root, Heap heap, String viewID, HeapViewerNodeFilter viewFilter, List<DataType> dataTypes, List<SortOrder> sortOrders, Progress progress) {
                if (selected != null) {
                    ...
                    return nodes == null || nodes.length == 0 ? new HeapViewerNode[] { new TextNode(getNoObjectsString()) } : nodes;
                }
    ...
    protected abstract HeapViewerNode[] getNodes(List<FieldValue> fields, HeapViewerNode parent, Heap heap, String viewID, HeapViewerNodeFilter viewFilter, List<DataType> dataTypes, List<SortOrder> sortOrders, Progress progress);
    
    protected abstract String getNoObjectsString();
    
    
    protected void nodeSelected(HeapViewerNode node, boolean adjusting) {
        DynamicObject selectedObject = node == null ? null : HeapViewerNode.getValue(node, DynamicObject.DATA_TYPE, heap);
