...
public class JvmstatProvider extends AbstractModelProvider<Jvmstat,Application> {
    ...
    static MonitoredVm getMonitoredVm(Application app) throws MonitorException {
        if (app.isFinished() || app.getPid() != Application.UNKNOWN_PID) return null;
    ...
    public Jvmstat createModelFor(Application app) {
        MonitoredVm vm = null;
        try {
            vm = getMonitoredVm(app);
            if (vm != null) {
                JvmstatImpl jvmstat = new JvmstatImpl(app,vm);
                app.notifyWhenFinished(jvmstat);
                return jvmstat;
            }
