...
var heapPkg = Packages.org.netbeans.lib.profiler.heap;
...
// Can take: FieldValue, JavaClass,
function wrapJavaValue(thing) {
//    if (thing == null || thing == undefined ||
//        thing instanceof heapPkg.model.HackJavaValue) {
//	return null;
//    }
    ...
    if (thing instanceof heapPkg.FieldValue) {
        // map primitive values to closest JavaScript primitives
        var type = thing.field.type;
        if (type instanceof heapPkg.PrimitiveType) {
...
//        if (jobject instanceof heapPkg.model.JavaObjectRef) {
//            jobject = jobject.dereference();
//            if (jobject instanceof heapPkg.model.HackJavaValue) {
//                print(jobject);
//                return null;
//            }
//        }

        if (jobject instanceof heapPkg.Instance) {
            return new JavaObjectWrapper(jobject);
        } else if (jobject instanceof heapPkg.JavaClass) {
            return new JavaClassWrapper(jobject);
        } else if (jobject instanceof heapPkg.ObjectArrayInstance) {
            return new JavaObjectArrayWrapper(jobject);
        } else if (jobject instanceof heapPkg.PrimitiveArrayInstance) {
    ...
    function JavaObjectWrapper(instance) {
        ...
        var fields = instance.javaClass.fields;
...
function wrapHeapSnapshot(heap) {
    function getClazz(clazz) {
        ...
        if (type == "string") {
            clazz = heap.getJavaClassByName(clazz);
        } else if (type == "object") {
    ...
    return {
        ...
        forEachClass: function(callback) {
            ...
            var classes = this.snapshot.allClasses;
         ...
         */
        classes: function() {
            return wrapIterator(this.snapshot.allClasses, true);
        },
...
//                var instances = clazz.getInstances(includeSubtypes);
        ...
        findClass: function(name) {
            var clazz = this.snapshot.getJavaClassByName(name + '');
        ...
        findObject: function(id) {
            return wrapJavaValue(this.snapshot.getInstanceByID(id));
        },
