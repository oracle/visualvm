...
public abstract class AbstractBarChartModel implements BarChartModel {
    ...
    final private Set<ChartModelListener> listeners = new HashSet<ChartModelListener>(); // Data change listeners
    ...
    protected void fireChartDataChanged() {
        Set<ChartModelListener> toNotify;

        synchronized (listeners) {
            toNotify = new HashSet<ChartModelListener>(listeners);
        }

        for(ChartModelListener listener : toNotify) {
            listener.chartDataChanged();
        }
    }
