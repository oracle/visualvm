 ...
 */
public class SynchronousXYChartAAA extends JComponent implements ComponentListener, ChartModelListener, MouseListener,
...
//    private double[] scaleFactorsY; // vertical scale factors for each series
    ...
    private long lastMaxY = Long.MIN_VALUE; // last global Y-value maximum
    private long lastMinY = Long.MAX_VALUE; // last global Y-value minimum
    private long dataOffsetY = 0L;
    ...
    public SynchronousXYChartAAA() {
        this(TYPE_FILL);
    }

    /** Creates a new instance of SynchronousXYChart */
    public SynchronousXYChartAAA(int type) {
        this(type, VALUES_INTERPOLATED);
    }

    public SynchronousXYChartAAA(int type, int dataType) {
        this(type, dataType, 1.0d);
    }

    public SynchronousXYChartAAA(int type, int dataType, double initialZoom) {
        this.type = type;
    ...
    public long getDataToViewHeight(long height, int seriesIndex) {
        return (long) Math.ceil(height * scaleFactorY);
    }
    ...
    public long getDataToViewY(long yValue, int seriesIndex) {
        return chartHeight
               - (long) Math.ceil(((yValue - dataOffsetY) * scaleFactorY) - chartInsets.top);
    }
...
//        scaleFactorsY = new double[model.getSeriesCount()];
    ...
    public static void main(String[] args) {
        SynchronousXYChartAAA xyChart = new SynchronousXYChartAAA(SynchronousXYChartAAA.TYPE_FILL);
    ...
    public void paintComponent(Graphics g) {
        ...
        // update series dependent arrays
//        long[] lastMinYsNew = new long[model.getSeriesCount()];
//        long[] lastMaxYsNew = new long[model.getSeriesCount()];
//
//        long[] dataOffsetsYNew = new long[model.getSeriesCount()];
//        double[] scaleFactorsYNew = new double[model.getSeriesCount()];
//        
//        System.arraycopy(lastMinYs, 0, lastMinYsNew, 0, Math.min(lastMinYs.length, lastMinYsNew.length));
//        System.arraycopy(lastMaxYs, 0, lastMaxYsNew, 0, Math.min(lastMaxYs.length, lastMaxYsNew.length));
//        System.arraycopy(dataOffsetsY, 0, dataOffsetsYNew, 0, Math.min(dataOffsetsY.length, dataOffsetsYNew.length));
//        System.arraycopy(scaleFactorsY, 0, scaleFactorsYNew, 0, Math.min(scaleFactorsY.length, scaleFactorsYNew.length));
//        
//        lastMinYs = lastMinYsNew;
//        lastMaxYs = lastMaxYsNew;
//        dataOffsetsY = dataOffsetsYNew;
//        scaleFactorsY = scaleFactorsYNew;
    ...
    private void drawChartLegend(Graphics2D g2, Shape chartClip, int startClipX, int endClipX) {
        ...
        for(int i=1;i<model.getSeriesCount();i++) {
            firstValueV = Math.min(firstValueV, model.getMinDisplayYValue(i) / (double) verticalAxisValueDivider);
            lastValueV = Math.max(lastValueV, model.getMaxDisplayYValue(i) / (double) verticalAxisValueDivider);
        }
    ...
    private void drawVerticalChartLegend(Graphics2D g2, Shape chartClip, int startClipX, int endClipX, double firstValue,
        ...
        if (useSecondaryVerticalAxis && !lastScaleYValid) {
            ...
            firstValue = model.getMinDisplayYValue(0) / (double) div;
            lastValue = model.getMaxDisplayYValue(0) / (double) div;
            for(int i=0;i<model.getSeriesCount();i++) {
                firstValue = Math.min(firstValue, model.getMinDisplayYValue(i) / (double) div);
                lastValue = Math.max(lastValue, model.getMaxDisplayYValue(i) / (double) div);
            }
    ...
    private void updateScaleFactors() {
        ...
        long maxY = lastMaxY;
        for (int seriesIndex = 0; seriesIndex < model.getSeriesCount(); seriesIndex++) {
            minY =  Math.min(minY, model.getMinDisplayYValue(seriesIndex));
            maxY = Math.max(maxY, model.getMaxDisplayYValue(seriesIndex));
        }
        
        if (lastMaxY != maxY || lastMinY != minY) {
            dataOffsetY = minY;
            scaleFactorY = (double) (chartHeight - topChartMargin) / (double) (maxY - dataOffsetY);
            yScaleChanged = true;
        }
        lastMaxY = maxY;
        lastMinY = minY;
