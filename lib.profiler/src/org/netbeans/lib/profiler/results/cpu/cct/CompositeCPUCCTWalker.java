 ...
 */
public class CompositeCPUCCTWalker extends CPUCCTWalker {
    //~ Instance fields ----------------------------------------------------------------------------------------------------------

    private List /*<CPUCCTVisitor>*/ visitors;

    //~ Constructors -------------------------------------------------------------------------------------------------------------

    /**
     * Creates a new instance of CompositeCPUCCTWalker
     */
    public CompositeCPUCCTWalker() {
        visitors = new ArrayList /*<CPUCCTVisitor>*/();
    }

    //~ Methods ------------------------------------------------------------------------------------------------------------------

    public void add(int position, CPUCCTVisitor visitor) {
        if (!visitors.contains(visitor)) {
            visitors.add(position, visitor);
        }
    }

    public void remove(CPUCCTVisitor visitor) {
        if (visitors.contains(visitor)) {
    ...
    // TODO Remove the hardcoded beforeWalk/afterWalk
    public void visit(SimpleCPUCCTNode rootNode) {
        for (Iterator iter = visitors.iterator(); iter.hasNext();) {
            CPUCCTVisitor visitor = (CPUCCTVisitor) iter.next();
